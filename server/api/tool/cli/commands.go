...

{
   ...
   "username": "xihnipyk1l"
	...
	tmp70 := new(CallbackTwitterCommand)
	sub = &cobra.Command{
	...
	tmp71 := new(CheckFirmwareCommand)
	sub = &cobra.Command{
	...
	tmp114 := new(ListDeviceDataFilesFilesCommand)
	sub = &cobra.Command{
	...
	tmp115 := new(ListDeviceLogFilesFilesCommand)
	sub = &cobra.Command{
	...
	tmp116 := new(ListDevicesFilesCommand)
	sub = &cobra.Command{
...
func (cmd *JSONFilesCommand) Run(c *client.Client, args []string) error {
	...
	if cmd.Dl != "" {
		var err error
		tmp146, err = boolVal(cmd.Dl)
		if err != nil {
			goa.LogError(ctx, "failed to parse flag into *bool value", "flag", "--dl", "err", err)
			return err
		}
	}
	resp, err := c.JSONFiles(ctx, path, tmp146)
	if err != nil {
		goa.LogError(ctx, "failed", "err", err)
		return err
	}
...
func (cmd *RawFilesCommand) Run(c *client.Client, args []string) error {
	...
	if cmd.Dl != "" {
		var err error
		tmp147, err = boolVal(cmd.Dl)
		if err != nil {
			goa.LogError(ctx, "failed to parse flag into *bool value", "flag", "--dl", "err", err)
			return err
		}
	}
	resp, err := c.RawFiles(ctx, path, tmp147)
	if err != nil {
		goa.LogError(ctx, "failed", "err", err)
		return err
	}
