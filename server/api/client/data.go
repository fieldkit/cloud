...
func (c *Client) NewDeviceDataDataRequest(ctx context.Context, path string, firstBlock *int, lastBlock *int, page *int, pageSize *int) (*http.Request, error) {
	...
	values := u.Query()
	if firstBlock != nil {
		tmp47 := strconv.Itoa(*firstBlock)
		values.Set("firstBlock", tmp47)
	}
	if lastBlock != nil {
		tmp48 := strconv.Itoa(*lastBlock)
		values.Set("lastBlock", tmp48)
	}
	if page != nil {
		tmp49 := strconv.Itoa(*page)
		values.Set("page", tmp49)
	}
	if pageSize != nil {
		tmp50 := strconv.Itoa(*pageSize)
		values.Set("pageSize", tmp50)
	}
