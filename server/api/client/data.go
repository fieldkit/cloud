...
func (c *Client) NewDeviceDataDataRequest(ctx context.Context, path string, firstBlock *int, lastBlock *int, page *int, pageSize *int) (*http.Request, error) {
	...
	values := u.Query()
	if firstBlock != nil {
		tmp61 := strconv.Itoa(*firstBlock)
		values.Set("firstBlock", tmp61)
	}
	if lastBlock != nil {
		tmp62 := strconv.Itoa(*lastBlock)
		values.Set("lastBlock", tmp62)
	}
	if page != nil {
		tmp63 := strconv.Itoa(*page)
		values.Set("page", tmp63)
	}
	if pageSize != nil {
		tmp64 := strconv.Itoa(*pageSize)
		values.Set("pageSize", tmp64)
	}
