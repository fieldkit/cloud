...
func (c *Client) NewDeviceDataDataRequest(ctx context.Context, path string, firstBlock *int, lastBlock *int, page *int, pageSize *int) (*http.Request, error) {
	...
	values := u.Query()
	if firstBlock != nil {
		tmp53 := strconv.Itoa(*firstBlock)
		values.Set("firstBlock", tmp53)
	}
	if lastBlock != nil {
		tmp54 := strconv.Itoa(*lastBlock)
		values.Set("lastBlock", tmp54)
	}
	if page != nil {
		tmp55 := strconv.Itoa(*page)
		values.Set("page", tmp55)
	}
	if pageSize != nil {
		tmp56 := strconv.Itoa(*pageSize)
		values.Set("pageSize", tmp56)
	}
