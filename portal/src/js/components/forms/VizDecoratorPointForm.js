        ...
        return (
            <div className='point-decorator'>
                <div className='decorator-row'>
                    <FormSelectItem labelText={ 'Location' } name={ 'location' } value={ data.points.location } inline={ false } firstOptionText={ 'Select' }
                        options={ target_attrs } errors={ errors } onChange={ this.setLocationKey } />
                </div>
                <div className='decorator-row'>
                    <FormSelectItem labelText={ 'Color' } name={ 'color' } value={ data.points.color.type } inline={ true } firstOptionText={ 'Select' }
                        options={ options } errors={ errors } onChange={ this.toggleColorType } />
                    { data.points.color.type !== 'constant' &&
                      
                      <FormSelectItem labelText={ 'Based on' } name={ 'color-data-key' } value={ data.points.color.data_key || '' } inline={ true } firstOptionText={ 'Select' }
                          options={ target_attrs } errors={ errors } onChange={ this.updateColorDataKey } /> }
                ...
                <div className='decorator-row'>
                    <FormSelectItem labelText={ 'Size' } name={ 'size' } value={ data.points.size.type } inline={ true } firstOptionText={ 'Select' }
                        options={ options } errors={ errors } onChange={ this.toggleSizeType } />
                    { data.points.size.type === 'constant' &&
                      <FormItem labelText={ 'Value' } name={ 'value' } value={ data.points.size.bounds[0] } inline={ true } errors={ errors } onChange={ e => this.setSize(e) }
                      /> }
                    { data.points.size.type !== 'constant' &&
                      <div>
                          <FormSelectItem labelText={ 'Based on' } name={ 'side-data-key' } value={ data.points.size.data_key || '' } inline={ true } firstOptionText={ 'Select' }
                              options={ target_attrs } errors={ errors } onChange={ this.updateSizeDataKey } />
                          <FormItem labelText={ 'Value' } name={ 'value' } value={ data.points.size.bounds[0] } inline={ true } errors={ errors } onChange={ e => this.setLowerSize(e) }
                          />
                          <FormItem labelText={ 'Max' } name={ 'upper-size' } value={ data.points.size.bounds[1] } inline={ true } errors={ errors } onChange={ e => this.setUpperSize(e) }
                          />
                      </div> }
                </div>
                <div className='decorator-row'>
                    <FormItem labelText={ 'Sprite' } name={ 'sprite' } value={ data.points.sprite } errors={ errors } onChange={ this.setSprite } />
                </div>
