    ...
    render() {
        ...
        return (
            <div className="collections">
                <Route path={ `${match.url}/new-collection` } render={ () => <ReactModal isOpen={ true } contentLabel="Create New Collection" className="modal" overlayClassName="modal-overlay">
                                                                                 <h2>Create New Collection</h2>
                                                                                 <ProjectCollectionsForm onCancel={ () => this.props.history.push(`${match.url}`) } onSave={ this.onCollectionCreate.bind(this) } />
                                                                             </ReactModal> } />
                <Route path={ `${match.url}/:collectionId/edit` } render={ props => <ReactModal isOpen={ true } contentLabel="Edit Collection" className="modal" overlayClassName="modal-overlay">
                                                                                        <ProjectCollectionsForm collection={ this.getCollectionById(parseInt(props.match.params.collectionId)) } onCancel={ () => this.props.history.push(`${match.url}`) } onSave={ this.onCollectionUpdate.bind(this, parseInt(props.match.params.collectionId)) } />
                                                                                    </ReactModal> } />
                { collectionDeletion &&
                  <ReactModal isOpen={ true } contentLabel="Delete Collection" className="modal" overlayClassName="modal-overlay">
                      <h2>Delete Team</h2>
                      <FormContainer onSave={ this.confirmCollectionDelete.bind(this) } onCancel={ this.confirmCollectionDelete.bind(this) } saveText="Confirm">
                          <div>
                              { collectionDeletion.contents }
                          </div>
                      </FormContainer>
                  </ReactModal> }
                <h1>Collections</h1>
                <div id="collections">
                    { collections.map((collection, i) => <div className="accordion-row-header">
