    ...
    render() {
        ...
        return (
            <div className="project">
                <Route path={ `${match.url}/add-administrator` } render={ props => <ReactModal isOpen={ true } contentLabel="Add Users" className="modal" overlayClassName="modal-overlay">
                                                                                       <h2>Add User</h2>
                                                                                       <AdministratorForm project={ project } administrators={ this.state.administrators } onCancel={ () => this.props.history.push(`${match.url}`) } onSave={ this.onAdministratorAdd.bind(this) } saveText="Add" />
                                                                                   </ReactModal> } />
                { administratorDeletion &&
                  <ReactModal isOpen={ true } contentLabel="Remove User" className="modal" overlayClassName="modal-overlay">
                      <h2>Remove User</h2>
                      <FormContainer onSave={ this.confirmAdministratorDelete.bind(this) } onCancel={ this.cancelAdministratorDelete.bind(this) } saveText="Confirm">
                          <div>
                              { administratorDeletion.contents }
                          </div>
                      </FormContainer>
                  </ReactModal> }
                ...
                <div className="row">
                    <div className="two-columns">
                        <h3>Main</h3>
                        <ProjectForm name={ project ? project.name : undefined } slug={ project ? project.slug : undefined } description={ project ? project.description : undefined } onSave={ this.onProjectSave.bind(this) } />
                    </div>
                    <div className="two-columns">
                        <h3>Users</h3>
                        <p>
                            Users you add to this project have administrative rights. They can create a new expedition, change its settings and add members to it. If you’re trying to add a
                            member to an expedition instead, select an expedition and then go to <i>Teams</i>.
                        </p>
                        <table className="administrators-table">
                            <thead>
                                <tr>
                                    <th></th>
                                    <th>Users (
                                        { administrators.length })</th>
                                    <th></th>
                                </tr>
                            </thead>
                            <tbody>
                                { administrators.map((administrator, i) => <tr key={ i }>
                                                                               <td>
                                                                                   ...
                                                                                   { users[administrator.user_id] &&
                                                                                     <div>
                                                                                         ...
                                                                                         <p className="type-small">
                                                                                             { users[administrator.user_id].username }
                                                                                         </p>
                                                                                     </div> }
                                                                               </td>
                                                                               <td>
