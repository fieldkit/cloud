...
export class Main extends Component {
    ...
    componentWillReceiveProps(nextProps: Props) {
        ...
        const {projectSlug, expeditionSlug} = this.props.match.params;

        const {projectSlug: newProjectSlug, expeditionSlug: newExpeditionSlug} = nextProps.match.params;
    ...
    async loadProjects() {
        ...
        if (projectRes.type === 'ok') {
            this.setState({
                projects: projectRes.payload.projects
            });
        } else {
            this.setState({
                projects: null
    ...
    async loadExpeditions(projectSlug: ?string) {
        ...
        if (projectSlug) {
            ...
            if (expRes.type === 'ok') {
                this.setState({
                    expeditions: expRes.payload.expeditions
                });
            }
        } else {
            this.setState({
    ...
    async loadActiveProject(projectSlug: ?string) {
        ...
        if (projectSlug) {
            ...
            if (projectRes.type === 'ok') {
                this.setState({
                    activeProject: projectRes.payload
                });
            }
        } else {
            this.setState({
    ...
    async loadActiveExpedition(projectSlug: ?string, expeditionSlug: ?string) {
        ...
        if (projectSlug && expeditionSlug) {
            ...
            if (expRes.type === 'ok') {
                this.setState({
                    activeExpedition: expRes.payload
                });
            }
        } else {
            this.setState({
    ...
    render() {
        const {user, activeProject, activeExpedition} = this.state;

        let {projects, expeditions} = this.state;
            ...
            <div className="main">
                        ...
                        { activeProject &&
                          <div className="breadcrumbs">
                              <div className="project-name">
                                  <NavLink exact to={ `/projects/${activeProject.slug}` }>
                                      { activeProject.name }
                                  </NavLink>
                              </div>
                              { activeExpedition &&
                                <div className="expedition-name-container">
                                    ...
                                    <div className="expedition-name">
                                        <NavLink exact to={ `/projects/${activeProject.slug}/expeditions/{activeExpedition.slug}` }>
                ...
                <div className="page-body row">
                    <div className="container content">
                        <Switch>
                            <RouteOrLoading path="/projects/:projectSlug/expeditions/:expeditionSlug/teams" component={ Teams } required={ [activeProject, activeExpedition] } project={ activeProject }
                                expedition={ activeExpedition } />
                            <RouteOrLoading path="/projects/:projectSlug/expeditions/:expeditionSlug/datasources" component={ DataSources } required={ [activeProject, activeExpedition] } project={ activeProject }
                                expedition={ activeExpedition } />
                            <RouteOrLoading path="/projects/:projectSlug/expeditions/:expeditionSlug" component={ Expedition } required={ [activeProject, activeExpedition] } project={ activeProject }
                                expedition={ activeExpedition } onUpdate={ this.onExpeditionUpdate.bind(this) } />
                            <RouteOrLoading path="/projects/:projectSlug/settings" component={ ProjectSettings } required={ [activeProject] } project={ activeProject } user={ user }
                                onUpdate={ this.onProjectUpdate.bind(this) } />
                            <RouteOrLoading path="/projects/:projectSlug/collections" component={ ProjectCollections } required={ [activeProject, expeditions] } project={ activeProject } />
                            <RouteOrLoading path="/projects/:projectSlug/visualizations" component={ ProjectVisualizations } required={ [activeProject] } project={ activeProject } />
                            <RouteOrLoading path="/projects/:projectSlug" component={ ProjectExpeditions } required={ [activeProject, expeditions] } project={ activeProject } expeditions={ expeditions }
                                onUpdate={ this.onProjectUpdate.bind(this) } onExpeditionCreate={ this.onExpeditionCreate.bind(this) } />
                            <RouteOrLoading path="/profile" component={ Profile } required={ [user] } user={ user } onUpdate={ this.onUserUpdate.bind(this) } />
                            <RouteOrLoading path="/" component={ Projects } required={ [projects] } projects={ projects } onProjectCreate={ this.onProjectCreate.bind(this) } />
                        </Switch>
                    </div>
