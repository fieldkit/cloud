...
export default class VizSelectionForm extends Component {
    ...
    render() {
        ...
        return (
            <div>
                <FormItem labelText={ 'Name' } name={ 'name' } value={ this.state.value_name } inline={ false } errors={ errors } onChange={ this.updateSelectionName }
                />
                <FormSelectItem labelText={ 'Selection Operation' } name={ 'selection_operation' } value={ this.state.operation } inline={ false } firstOptionText={ 'Select' }
                    options={ opts } errors={ errors } onChange={ this.updateSelectionOperation } />
                <FormSelectItem labelText={ 'Selection Attribute' } name={ 'selection_attribute' } value={ this.state.source_attribute.name } inline={ false } firstOptionText={ 'Select' }
                    options={ attrs } errors={ errors } onChange={ this.updateSelectionSource } />
                <div>
                    <button onClick={ () => creator.addSelection(this.state) }>Save</button>
                    <button onClick={ () => creator.setState({
                                          modal_open: false
                                      }) }>
                        Cancel
                    </button>
