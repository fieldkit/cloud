...
export class FKApiClient extends APIClient {
  ...
  async getCurrentPerson(): Promise<PersonResponse> {
    ...
    const res: PersonResponse = await this.getJSON('http://fieldkit.org/api/person/current');
  ...
  async register(params: Object): Promise<void> {
    await this.postForm('http://fieldkit.org/api/user/sign-up', params);
  }

  async login(username: string, password: string): Promise<void> {
    // response has no content, so any non-error means success
    await this.postForm('http://fieldkit.org/api/user/sign-in', { username, password });
  }

  async logout(): Promise<void> {
    await this.postForm('http://fieldkit.org/api/user/logout');
    this.onLogout();
  }

  async getProjects () {
    const res = await this.getJSON('http://fieldkit.org/api/projects')
    return res
  }

  async createProjects (name) {
    const res = await this.getJSON('http://fieldkit.org/api/projects/add?name=' + name)
    return res
  }

  async getExpeditions (projectID) {
    const res = await this.getJSON('http://fieldkit.org/api/project/' + projectID + '/expeditions')
    return res
  }

  async postGeneralSettings (projectID, expeditionName) {
    const res = await this.getJSON('http://fieldkit.org/api/project/' + projectID + '/expeditions/add?name=' + expeditionName)
    return res
  }

  async postInputs (projectID, expeditionID, inputName) {
    const res = await this.getJSON('http://fieldkit.org/api/project/' + projectID + '/expedition/' + expeditionID + '/inputs/add?name=' + inputName)
    return res
  }
