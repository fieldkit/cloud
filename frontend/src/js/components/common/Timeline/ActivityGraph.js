...
class ActivityGraph extends React.Component {
    ...
    render() {
        const {documents, startDate, endDate, lineWidth, x, y, w, h, rad} = this.props
        ...
        return (
            <g style={ { transform: `translate(${x}px, ${y}px)` } }>
                { buckets.map((b, i) => {
                      ...
                      return (
                          <rect key={ i } x={ -width / 2 + w / 2 } y={ map(i, 0, bucketCount, 0, h) } width={ width } height={ bucketHeight } fill="white" />
                      )
                  }) }
            </g>
        )
