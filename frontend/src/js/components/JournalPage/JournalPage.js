...
class JournalPage extends React.Component {
    ...
    updateDate() {
        ...
        const {documents, updateDate} = this.props
    ...
    updateScrollTop(prevProps, prevState, didMount) {
        const {documents, currentDate} = this.props
        const {postDimensions} = this.state
    ...
    render() {
        const {documents, currentExpeditionID, currentDate, updateDate} = this.props

        const {postMarginBottom, contentPadding} = this.state
        ...
        return (
            <div className="journal-page page">
                ...
                <div className="journal-page_content" onScroll={ this.onScroll } ref="content">
                    <ul className="journal-page_content_posts" ref="posts">
                        { documents
                              .map(d => {
                                  return (
                                      <li className="journal-page_content_posts_post" key={ 'post-' + d.get('id') } style={ { marginBottom: postMarginBottom } }>
                                          <Post data={ d } currentExpeditionID={ currentExpeditionID } updateDate={ updateDate } ref={ 'post-' + d.get('id') } />
                                      </li>
                                  )
                              }) }
                    </ul>
                    <div style={ { height: contentPadding } } />
                </div>
            </div>
        )
    }
